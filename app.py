...
def process_video():
    try:
        app.logger.info(f"[process_video] Request received")
        ...
        app.logger.info(f"[process_video] Received files: video={video_file.filename if video_file else None}, audio={audio_file.filename if audio_file else None}, text_length={len(text_content)}")
        ...
        app.logger.info(f"[process_video] Created temp dir: {temp_dir}")
        ...
        app.logger.info(f"[process_video] Saved video and audio files")

        bgm_path = None
        if bgm_file:
            bgm_path = temp_dir / bgm_file.filename
            bgm_file.save(str(bgm_path))
            app.logger.info(f"[process_video] Saved BGM file")
        ...
        try:
            # 调用视频处理模块
            app.logger.info(f"[process_video] Starting video processing")
            ...
            app.logger.error(f"[process_video] Processing error: {str(e)}")
            shutil.rmtree(temp_dir, ignore_errors=True)
            return jsonify({'code': 2, 'msg': f'Processing failed: {str(e)}'})
        ...
        app.logger.error(f"[process_video] error: {str(e)}")
        import traceback
        app.logger.error(f"[process_video] traceback: {traceback.format_exc()}")
